...
export class StringArrayName extends AbstractName {
    ...
    protected assertStringArrayNameInvariants() {
        ...
        InvalidStateException.assertIsNotNullOrUndefined(this.components, "this.components is null or undefined");
    ...
    protected assertCorrectState(components: string[], delimiter: string | undefined): void {
        ...
        MethodFailedException.assertCondition(cond, "StringArrayName not correctly consructed");
    }
    ...
    protected assertSetComponent(c: string, index: number, original: string[]): void {
        if (original.length !== this.components.length) {
            ...
            MethodFailedException.assertCondition(false, `component was not set correctly`);
        }
        ...
        for (let i = 0; i < original.length; i++) {
            if(this.components[i] !== original[i]) {
                ...
                if (cond === false) {
                    this.restoreStringArrayName(original);
                    MethodFailedException.assertCondition(cond, `component was not set correctly`);
                }
            }
        }
    }
    ...
    protected assertInsertComponent(c: string, index: number, original: string[]): void {
        if (original.length !== this.components.length - 1) {
            this.restoreStringArrayName(original);
            MethodFailedException.assertCondition(false, `component was not inserted`);
        }
        ...
        for (let i_orig = 0; i_orig < original.length; i_orig++) {
            if (i_orig === index) {
                if(this.components[i_new] !== c) {
                    cond = false;
                    this.restoreStringArrayName(original);
                    MethodFailedException.assertCondition(cond, `component was not inserted`);
                }
                i_new++;
            }
            if(this.components[i_new] !== original[i_orig]) {
                cond = false;
                this.restoreStringArrayName(original);
                MethodFailedException.assertCondition(cond, `component was not removed`);
            }
            i_new++;
        }
    }
    ...
    protected assertAppendComponent(c: string, original: string[]): void {
        if (original.length !== this.components.length - 1) {
            this.restoreStringArrayName(original);
            MethodFailedException.assertCondition(false, `component was not appended`);
        }
        ...
        for (let i = 0; i < original.length; i++) {
            if(this.components[i] !== original[i]) {
                cond = false;
                this.restoreStringArrayName(original);
                MethodFailedException.assertCondition(cond, `component was not appended`);
            }
        }
    ...
    protected assertRemovedComponent(index: number, original: string[]): void {
        if (original.length !== this.components.length + 1) {
            this.restoreStringArrayName(original);
            MethodFailedException.assertCondition(false, `component was not removed`);
        }
    ...
    protected assertValidComponents(): void {
        this.components.forEach(c => {
            InvalidStateException.assertIsNotNullOrUndefined(c, "at least one of the components is null or undefined");
        });
    }
}